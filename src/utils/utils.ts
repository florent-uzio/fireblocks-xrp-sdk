...
import { ClientError, ValidationError } from "../errors/errors.js"
import { MAX_DOMAIN_BYTES, TTL_CONST, XRP_LIMITS } from "./constants.js"
...
export const estimateNetworkFees = async (client: Client): Promise<string> => {
  try {
    ...
    if (!state) {
      throw new ClientError("XrpClientError", "No server state response received")
    }
    ...
    throw new Error(
      `Failed to estimate network fees: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}
...
export const getSequencesAndFee = async (
  ...
  try {
    ...
    if (!address || typeof address !== "string") {
      throw new ValidationError("InvalidAddress", "Valid address string is required")
    }
    ...
    throw new Error(
      `Failed to get network parameters: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
}
...
export const getNetworkParams = async (
  ...
  let lastError: Error = new ClientError("UnknownError", "Unknown client error occurred")
 ...
 */
export const deriveOfferCreateFlags = (flags: IOfferCreateFlags): number | undefined => {
  if (!flags) {
    return undefined
  }
  ...
  if (flags.tfImmediateOrCancelOffer) combined |= OfferCreateFlags.tfImmediateOrCancel
 ...
 * @returns The combined bitmask or undefined if no flags are set
 */
export const derivePaymentFlags = (flags: IPaymentFlags): number | undefined => {
  if (!flags) {
    return undefined
  }
...
export const validateAmount = (name: string, amt: Amount): void => {
  ...
  if (amt === null || amt === undefined) {
    throw new ValidationError("MissingAmount", `${name} cannot be null or undefined`)
  }
  ...
  if (typeof amt === "string") {
    ...
    if (!/^\d+$/.test(amt)) {
      throw new ValidationError("InvalidAmount", `${name} as XRP must be a string of integer drops`)
    }
    ...
    try {
      ...
      if (amountBigInt <= 0n) {
        throw new ValidationError("InvalidAmount", `${name} must have a positive amount as value`)
      }
  ...
  // Validate token amount (object with currency, issuer, value)
    ...
    if (typeof currency !== "string") {
      throw new ValidationError("InvalidAmount", `${name} currency must be a string`)
    }
    ...
    if (typeof issuer !== "string" || !isValidAddress(issuer)) {
      throw new ValidationError("InvalidAmount", `${name} issuer is not a valid XRPL address`)
    }

    // Validate value is a properly formatted number string
    if (typeof value !== "string" || !/^-?\d*\.?\d+$/.test(value)) {
      throw new ValidationError("InvalidAmount", `${name} value must be a valid number string`)
    }

    // Validate value is positive
    if (parseFloat(value) <= 0) {
      throw new ValidationError("InvalidAmount", `${name} must be a positive amount`)
    }
  } else {
    throw new ValidationError(
      "InvalidAmount",
...
export const validateDestinationTag = (tag: number): void => {
  ...
  if (typeof tag !== "number" || !Number.isInteger(tag) || tag < 0 || tag > 0xffffffff) {
    throw new ValidationError(
...
export const validateMemos = (memos: any[]): Memo[] | undefined => {
  ...
  return memos.map((memo) => {
    ...
    if (!memo || typeof memo !== "object") {
      throw new ValidationError("InvalidMemo", "Each memo must be a non-null object")
    }
...
export function deriveAccountSetAsf(
  ...
  const setKeys = set ? (Object.keys(set) as (keyof AsfFlags)[]).filter((k) => set[k]) : []
  const clearKeys = clear ? (Object.keys(clear) as (keyof AsfFlags)[]).filter((k) => clear[k]) : []

  if (setKeys.length > 1)
    throw new ValidationError("InvalidAsfFlags", `Cannot set more than one ASF flag: ${setKeys}`)
...
export function hexEncodeDomain(domain: string): string {
  ...
  if (hex.length > MAX_DOMAIN_BYTES * 2) {
    throw new ValidationError("InvalidDomain", "Domain too long; max 256 bytes.")
  }
  return hex
}
