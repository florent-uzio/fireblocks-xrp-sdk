import { BasePath, type TransactionResponse } from "@fireblocks/ts-sdk"
...
import { type ApiServiceConfig, type SdkManagerMetrics, TransactionType } from "../pool/types.js"
...
export class FbksXrpApiService {
  ...
  constructor(config: ApiServiceConfig) {
    ...
    if (!config.apiKey || typeof config.apiKey !== "string" || !config.apiKey.trim()) {
      throw new ValidationError("InvalidConfig", "apiKey must be a non-empty string.")
    }
    if (!config.apiSecret || typeof config.apiSecret !== "string" || !config.apiSecret.trim()) {
      throw new ValidationError("InvalidConfig", "apiSecret must be a non-empty string.")
    }
    if (!config.assetId || typeof config.assetId !== "string" || !config.assetId.trim()) {
      throw new ValidationError("InvalidConfig", "assetId must be a non-empty string.")
    }
    if (config.basePath && !Object.values(BasePath).includes(config.basePath as BasePath)) {
    ...
    if (config.poolConfig && typeof config.poolConfig !== "object") {
      throw new ValidationError("InvalidConfig", "poolConfig must be an object if provided.")
    }
    ...
    try {
      ...
      switch (transactionType) {
        ...
        case TransactionType.CROSS_CURRENCY_PAYMENT:
          result = await sdk.crossCurrencyPayment(params as CrossCurrencyPaymentOpts)
          break
        ...
        case TransactionType.XRP_TRANSFER:
          ...
          break
        default:
          logger.error(`Unknown transaction type: ${transactionType} for vault ${vaultAccountId}`)
          throw new ValidationError("InvalidType", `Unknown transaction type: ${transactionType}`)
      }

      return result
    } catch (error) {
      logger.error(`Error executing ${transactionType} for vault ${vaultAccountId}:`, error)
      throw error
    } finally {
