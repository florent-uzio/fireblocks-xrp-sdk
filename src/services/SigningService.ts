...
import { hashSignedTx, hashTx } from "xrpl/dist/npm/utils/hashes/index.js"
import { FireblocksXrpSdk } from "../FireblocksXrpSdk.js"
import { SigningError } from "../errors/errors.js"
...
export class SigningService {
  ...
  public signAsync = async (
    ...
    try {
      ...
      return await this.getSignedTransaction(placeholder, assetId, vaultAccountId, note || "")
    } catch (err: any) {
      throw err instanceof SigningError
        ? err
        : new SigningError("SignAsyncFailed", `signAsync failed: ${err.message || err}`)
    }
  }
  ...
  public sign = (
    ...
    try {
      ...
      throw err instanceof SigningError
        ? err
        : new SigningError("SignFailed", `Error in sign(): ${err.message || err}`)
    }
  }
  ...
  private toDER = (rHex?: string, sHex?: string): string => {
    try {
      const rmPadding = (buf: number[]) => {
        ...
        // @ts-expect-error - buf[i + 1] may need to look into it later
      ...
      // @ts-expect-error - r[0] may need to look into it later
      if (r[0] & 0x80) r = [0].concat(r)
      // @ts-expect-error - s[0] may need to look into it later
      if (s[0] & 0x80) s = [0].concat(s)

      r = rmPadding(r)
      s = rmPadding(s)

      // @ts-expect-error - s[1] may need to look into it later
      while (!s[0] && !(s[1] & 0x80)) {
        s = s.slice(1)
      }
  ...
  public waitForSignature = async (
    ...
    try {
      ...
      logger.info(`Transaction ${tx.id} is currently at status - ${TransactionStateEnum.Completed}`)
  ...
  public getSignedTransaction = async (
    ...
    if (!prepared) {
      throw new SigningError("PlaceholderExpired", "Prepared transaction expired or not found")
    }
    ...
    try {
      ...
      if (!sig) {
        throw new SigningError("NoSignature", "No signature returned from Fireblocks")
      }
      ...
      throw error instanceof SigningError
        ...
        : new SigningError(
            ...
            `Error signing transaction with Fireblocks: ${error.message || error}`,
          )
    }
  }
}
