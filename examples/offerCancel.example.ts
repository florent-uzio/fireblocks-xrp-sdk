...
import { FbksXrpApiService, OfferCancelOpts, TransactionType } from "../src/"
...
;(async () => {
  ...
  try {
    const params: OfferCancelOpts = { offerSequence: 7855630 }
    ...
    if ("result" in res) {
      ...
      if (
        ...
        console.log(`Tx submission failed with result: ${res.result.meta.TransactionResult}`)
      } else {
