...
import { ClientError, ValidationError } from "../../src/errors/errors"
...
describe("Utils", () => {
  ...
  describe("estimateNetworkFees", () => {
    ...
    it("throws ClientError if server state is missing", async () => {
      ...
      await expect(estimateNetworkFees(mockClient)).rejects.toThrow(ClientError)
    })
  ...
  describe("getAccountAndLedgerSequences", () => {
    ...
    it("throws ValidationError for invalid address", async () => {
      await expect(getAccountAndLedgerSequences(mockClient, undefined as any)).rejects.toThrow(
        ValidationError,
      )
      await expect(getAccountAndLedgerSequences(mockClient, "")).rejects.toThrow(ValidationError)
    })
    it("throws ClientError if required fields missing", async () => {
      mockClient.request.mockResolvedValue({
        result: { account_data: {}, ledger_current_index: undefined },
      })
      await expect(getAccountAndLedgerSequences(mockClient, "rAddress")).rejects.toThrow(
        ClientError,
      )
    })
  ...
  describe("getSequencesAndFee", () => {
    it("throws ValidationError if client is missing", async () => {
      await expect(getSequencesAndFee(undefined as any, "rA")).rejects.toThrow(ValidationError)
    })
    it("throws ValidationError if address is missing", async () => {
      await expect(getSequencesAndFee(mockClient, "")).rejects.toThrow(ValidationError)
    })
    it("calls ensureConnection and returns network params", async () => {
      mockClient.isConnected.mockReturnValue(true)
      jest.spyOn(require("../../src/utils/utils"), "estimateNetworkFees").mockResolvedValue("11")
      jest
  ...
  describe("getNetworkParams", () => {
    ...
    it("does not retry on ValidationError", async () => {
      ...
      await expect(getNetworkParams(mockClient, "addr", 2, 1)).rejects.toThrow(ValidationError)
    })
  })
  ...
  describe("validateAmount", () => {
    ...
    it("throws ValidationError for invalid amount object", () => {
      expect(() => validateAmount("Amount", { currency: "USD", value: "100" } as any)).toThrow(
        ValidationError,
      )
    })
  })
  ...
  describe("deriveAccountSetAsf", () => {
    ...
    it("throws ValidationError for more than one flag in set or clear", () => {
      expect(() =>
        deriveAccountSetAsf({ asfRequireAuth: true, asfDisableMaster: true }, {}),
      ).toThrow(ValidationError)
      expect(() =>
        deriveAccountSetAsf({}, { asfRequireAuth: true, asfDisableMaster: true }),
      ).toThrow(ValidationError)
  ...
  describe("deriveAccountSetTf", () => {
    it("returns correct combined flag value or undefined if empty", () => {
      ...
      expect(deriveAccountSetTf({ tfRequireDestTag: true, tfDisallowXRP: true })).toBe(
  ...
  describe("derivePaymentFlags", () => {
    it("returns correct combined flag value or undefined if empty", () => {
      expect(derivePaymentFlags({ tfPartialPayment: true, tfLimitQuality: true })).toBe(
        (PaymentFlags.tfPartialPayment || 0) | (PaymentFlags.tfLimitQuality || 0),
      )
  ...
  describe("validateMemos", () => {
    ...
    it("throws ValidationError for memos missing all fields", () => {
      expect(() => validateMemos([{ NoMemoFields: true }])).toThrow(ValidationError)
    })
    ...
    it("throws ValidationError for invalid MemoData/Type/Format types", () => {
      expect(() => validateMemos([{ MemoData: 123, MemoType: "foo" }])).toThrow(ValidationError)
      expect(() => validateMemos([{ MemoType: 123, MemoData: "foo" }])).toThrow(ValidationError)
      expect(() => validateMemos([{ MemoFormat: 123, MemoData: "foo" }])).toThrow(ValidationError)
    })
  })
  describe("validateHash256", () => {
    ...
    it("throws ValidationError for non-hex or wrong length", () => {
      // Too short
      expect(() => validateHash256("DomainID", "abc123")).toThrow(ValidationError)

      // Too long
      expect(() => validateHash256("DomainID", "a".repeat(65))).toThrow(ValidationError)
      ...
      // Not string
      expect(() => validateHash256("DomainID", 123 as any)).toThrow(ValidationError)
    })
  })
})
