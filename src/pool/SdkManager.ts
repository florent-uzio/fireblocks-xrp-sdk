...
import { type FireblocksConfig, FireblocksXrpSdk } from "../FireblocksXrpSdk.js"
import { Logger } from "../utils/logger.js"
import type { PoolConfig, SdkManagerMetrics, SdkPoolItem } from "./types.js"
...
export class SdkManager {
  ...
  constructor(
    ...
    this.fireblocks = FireblocksXrpSdk.createFireblocksInstance(this.baseConfig)
    ...
    if (!poolItem) {
      ...
      if (!poolItem.sdk.client.isConnected()) {
        try {
          ...
          logger.error(`Failed to reconnect client for vault ${vaultAccountId}:`, error)
        }
      }
      poolItem.lastUsed = new Date()
      poolItem.isInUse = true
   ...
   */
  private createSdkInstance = async (vaultAccountId: string): Promise<FireblocksXrpSdk> => {
